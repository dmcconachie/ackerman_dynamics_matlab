function intersection = oar_line_intersection(obs, p1, p2)
% Only checks the lines that makeup the rectangle
    ...
    if norm(p1 - p2) < 1e-10
        ...
        XY1 = [p1', p2'];
        XY2 = [points(:, 1)', points(:, 2)';
               points(:, 2)', points(:, 3)';
               points(:, 3)', points(:, 4)';
               points(:, 4)', points(:, 1)'];
        test = line_line_intesect(XY1, XY2);
        intersection = any(test);
    end

end

function intersection = line_line_intesect(XY1, XY2)
